...
class Functions
{
    ...
    public function store($request)
    {
        ...
        if ($request->status == 2) {
            ...
            $atualTask = Approver::where('form_number', $createFormNumber->id)->orderBy('ordem', 'asc')->where('status', '1')->first();
    ...
    public function update($request)
    {
        ...
        if ($this->childs != null) {
            ...
            foreach ($request->childs as $child) {
                ...
                ucfirst(strtolower('App\Models\FormModels\\' . $this->childs))::create(collect($objetoChild)->toArray());
            }
        }
        ...
        if ($request->status == 2) {
            ...
            $atualTask = Approver::where('form_number', $editaForm->form_number)->orderBy('ordem', 'asc')->where('status', '1')->first();
    ...
    {
        $atualTask = Approver::where('form_number', $request->form_number)->orderBy('ordem', 'asc')->where('status', '1')->first();
        ...
        ucfirst(strtolower('App\Models\FormModels\\' . $this->model))::where('id', $request->id)
            ->update(collect($objeto)->toArray());
        ...
        if ($this->childs != null) {
            foreach ($request->childs as $child) {
                $objetoChild = (object)$child;
                unset($objetoChild->form_number);
                unset($objetoChild->create_user_id);
                unset($objetoChild->status);
                unset($objetoChild->childs);
                unset($objetoChild->created_at);
                unset($objetoChild->updated_at);
                unset($objetoChild->id);

                $Newchild = ucfirst(strtolower('App\Models\FormModels\\' . $this->childs))::where('id', $child['id'])
                    ->update(collect($objetoChild)->toArray());

                if ($this->childChilds != null) {
                    foreach ($child['childs'] as $childChild) {
                        $objetoChildChild = (object)$childChild;
                        unset($objetoChildChild->form_number);
                        unset($objetoChildChild->filho_id);
                        unset($objetoChildChild->create_user_id);
                        unset($objetoChildChild->status);
                        unset($objetoChildChild->created_at);
                        unset($objetoChildChild->updated_at);
                        unset($objetoChildChild->id);

                        ucfirst(strtolower('App\Models\FormModels\\' . $this->childChilds))::where('id', $childChild['id'])->update(collect($objetoChildChild)->toArray());
                    }
                }
            }
        }
        ...
        if ($atualTask != null) {
            ...
            if ($approver) {
                ...
                if ($proximaTarefa->type == 4) {
                    //finalizando o processo por fluxo de aprovaçãoF
                    ...
                    $nextTask = Approver::where('form_number', $request->form_number)->orderBy('ordem', 'asc')->where('status', '1')->first();

                    $atualiza = FormNumber::find($request->form_number);
                    $atualiza->atual_task = $nextTask->id;
                    $atualiza->task = $nextTask->name;
                    $atualiza->atual_group = $nextTask->group;
                    $atualiza->atual_user = $nextTask->user_id;
                    $atualiza->save();
